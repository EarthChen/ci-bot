...
- **入队稳定窗口**：出队前若同 serialKey 在 `CIHEAL_QUIET_WINDOW_MS`（默认 0 = 关闭）内有更新入队则延迟出队（分支连推时等稳定再修，减少 supersede/整轮作废浪费；实测：50 分钟修复中途被 supersede 三次、终局时源 MR 已 merge）；同 key 新入队重置窗口；运行中 steer 不受影响
