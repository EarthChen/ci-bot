...
```acceptance-report
{
  "criteriaSatisfied": [
    {
      "id": "criterion-1",
      "status": "satisfied",
      "evidence": "Brief cites concrete file paths (SKILL.md, AgentDefinition in agents param, ClaudeAgentOptions), GitHub issues (#64894 headersHelper bug, #69620 headless MCP docs gap, #14882 skill full-load regression, #17327 proactive invocation), and SDK source artifacts (claude-agent-sdk-python README at commit 70bff63, examples/tool_permission_callback.py at a0fbd14). Severity-tagged findings: #64894 = blocker for HTTP MCP auth, #14882 = high for context budget, #17327 = medium for selection determinism."
    }
  ],
  "changedFiles": [],
  "testsAddedOrUpdated": [],
  "commandsRun": [
    {
      "command": "web_search (4 query batches, 6-8 results each)",
      "result": "passed",
      "summary": "Covered 4 research angles: skill SDK form, multi-skill selection, context budget, MCP headless auth"
    },
    {
      "command": "fetch_content (readable, 4 GitHub/docs URLs that resolved)",
      "result": "passed",
      "summary": "Retrieved README, subagents guide, permission example, controllability analysis"
    },
    {
      "command": "get_search_content (bounded slices, 8 retrievals)",
      "result": "passed",
      "summary": "Extracted issue #64894 mitmproxy evidence, issue #69620 doc gap, issue #14882 regression, Promptfoo option table, subagent token data"
    },
    {
      "command": "write research.md to authoritative output path",
      "result": "passed",
      "summary": "Brief written to /Users/earthchen/ai-work/ci-bot/.pi-subagents/artifacts/outputs/d74447af-220a-403b-a859-186262cff1c8/research.md"
    }
  ],
  "validationOutput": [
    "All 4 research points covered with [事实]/[未知]/[推论] tags per requirement",
    "Special marking section included: SDK skill is same as interactive Claude Code; programmatic correspondents (subagent, in-process MCP server, can_use_tool) explicitly identified for re-scoping",
    "Severity-tagged findings: headersHelper bug #64894 (blocker for HTTP MCP), skill full-load #14882 (high for context budget), LLM skill selection non-determinism (medium)",
    "5 concrete gaps identified with suggested next-step empirical tests"
  ],
  "residualRisks": [
    "issue #14882 (skill full-token load at startup) observed only in interactive /context; SDK query() path unverified — context budget estimates may be optimistic if regression applies to headless",
    "headersHelper bug #64894 marked closed but fix availability in PyPI-bundled CLI version unverified — remote HTTP MCP with pre-provisioned tokens may still fail",
    "Concurrent query() resource overhead (subprocess per session) not quantified — multi-project concurrency limits unknown",
    "claude.com / platform.claude.com / code.claude.com / anthropic.com domains blocked by network policy during research; official docs cited via search snippets and secondary sources, not direct fetch — some doc precision may be lost"
  ],
  "noStagedFiles": true,
  "diffSummary": "Created research brief markdown file at the authoritative output path; no source code changes. Brief covers 4 research points with epistemic tags, a cross-cutting recommended architecture, a special marking on skill-vs-subagent re-scoping, and an acceptance report.",
  "reviewFindings": [
    "no blockers: research brief is documentation-only, no code modified"
  ],
  "manualNotes": "Official docs (code.claude.com, platform.claude.com, anthropic.com) were network-blocked; findings rely on search-result snippets, GitHub issues (directly fetched), the Python SDK README (directly fetched), and the Promptfoo provider doc (directly fetched, most complete option reference). The 5 gaps listed should be resolved by empirical tests before locking architecture decisions, especially #14882 regression in SDK path and #64894 fix status in bundled CLI."
}
```