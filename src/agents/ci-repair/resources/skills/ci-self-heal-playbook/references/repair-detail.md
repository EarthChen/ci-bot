...
1. 违规按规则/类别分组，机械项（导入、命名、格式、补注解、加 final 等）**批量修完再验证**——每处修完都跑一遍 mvn 是主要时间黑洞（全模块 compile 每次 2-3 分钟）。**编辑本身也必须聚合，禁止逐文件串行**（每处都要付一次模型 turn 推理时间 20-60s；实测几十处机械违规逐文件修 30+ 分钟，聚合后几分钟）。聚合方式按模式可信度二选一：**a) 脚本批量**——模式明确、命中无歧义时首选，一条 sed/python 扫完全部命中文件，1 turn 完成；盲替换的误命中风险靠 compile + 规则复跑 + `git diff` 复核兜底。**b) 并行 edit**——命中少或语境敏感（同名符号、条件替换）时用，同一条消息内对多个文件并行发起 edit（Pi 默认并行执行 tool call、per-file 队列保序），锚点校验精确，代价是先读文件、token 更多。
