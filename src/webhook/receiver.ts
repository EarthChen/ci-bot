...
export function parsePipelinePayload(body: unknown): PipelineEvent | null {
  ...
  const projectId = extractProjectId((obj.project ?? {}) as Record<string, unknown>);
  if (projectId === null) return null;
  const ref = attrs.ref;
  const sha = attrs.sha;
  if (typeof attrs.id !== "number" || typeof ref !== "string" || typeof sha !== "string") {
    return null;
  }
  return { projectId, pipelineId: attrs.id, ref, sha, projectUrl: extractProjectUrl((obj.project ?? {}) as Record<string, unknown>) };
}
...
export async function mountWebhook(
  ...
  app.post("/webhook", async (req: FastifyRequest, reply: FastifyReply) => {
    ...
    if (!rateAllowed(req.ip, deps.config.rateLimitMax, deps.config.rateLimitWindowMs)) {
