...
function pickAgent(): AgentRunner {
  const mode = process.env.CIHEAL_AGENT_MODE ?? "stub";
  if (mode === "stub") return new StubAgentRunner();
  // ticket 02+ lands the real pi runner here.
  throw new Error(`CIHEAL_AGENT_MODE=${mode} not supported in ticket 01`);
}
...
export async function main(): Promise<void> {
  ...
  try {
    ...
    throw new Error(`invalid CIHEAL_WORKER_TASK JSON: ${err instanceof Error ? err.message : err}`);
  }
